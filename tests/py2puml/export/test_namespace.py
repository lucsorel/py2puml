...
def test_build_packages_structure(uml_items: List[UmlItem], items_number_by_package_name: Dict[str, int]):
    ...
    for uml_item in uml_items:
        ...
        for module_name in module_parts:
            ...
            assert (
                expected_items_number == inner_package.items_number
            ), f'package {module_name} must have {expected_items_number} items, found {inner_package.items_number}'

            has_children_packages = len(inner_package.children) > 0
            has_module_items = inner_package.items_number > 0
            assert (
                has_children_packages or has_module_items
            ), f'package {inner_package.name} in hierarchy of {module_parts} has items or children packages'
...
@mark.parametrize(
    ...
    [
        (
            ...
            0,
            [],
        ),  # the root package yields no namespace documentation
        (
        ...
        (
            ...
            ('py2puml', 'domain'),
            0,
            ['namespace py2puml.domain.umlclass {}\n'],
        ),
        (
            SAMPLE_ROOT_PACKAGE,
            tuple(),
...
def test_build_packages_structure_visit_package_from_tree_package(
    ...
    with open(f'{domain_path}/plantuml_namespace.txt', encoding='utf8') as tree_namespace_file:
        for line_index, (namespace_line, expected_namespace_line) in enumerate(
            zip(visit_package(package, tuple(), 0), tree_namespace_file)
        ):
