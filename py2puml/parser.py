...
def filter_domain_definitions(module: ModuleType, root_module_name: str) -> Iterable[Type]:
    for definition_key in dir(module):
        ...
        if isclass(definition_type) or isfunction(definition_type):
...
def parse_type(
    ...
    if definition_type_fqdn not in domain_items_by_fqdn:
        if isfunction(definition_type):
            parse_func_type(definition_type, definition_type_fqdn, root_module_name, domain_items_by_fqdn, domain_relations)
