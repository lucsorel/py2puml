...
def remove_forward_references(compound_type_annotation: str, module_name: str) -> str:
    ...
    '''
    return None if compound_type_annotation is None else FORWARD_REFERENCES.sub(
        f'{module_name}.\\1', compound_type_annotation
    )
...
def replace_nonetype_occurrences_in_union_types(type_annotation: str) -> str:
    ...
    while (union_match_clauses := list(LAST_NONETYPE_IN_UNION.finditer(cleaned_type_annotation)
                                      )) and len(union_match_clauses) > 0:
        union_last_match_clause = union_match_clauses[-1]
        match_start, match_end = union_last_match_clause.span()
        cleaned_type_annotation = cleaned_type_annotation[0:match_end -
                                                          8] + 'None' + cleaned_type_annotation[match_end:]
