...
def parse_class_constructor(class_type: Type, class_fqn: str,
                            root_module_name: str) -> Tuple[List[UmlAttribute], Dict[str, UmlRelation]]:
    constructor = getattr(class_type, '__init__', None)
    # conditions to meet in order to parse the AST of a constructor
    if (  # the constructor must be defined
            constructor is None) or (  # the constructor's source code must be available
                not hasattr(constructor,
                            '__code__')) or (  # the constructor must belong to the parsed class (not its parent's one)
